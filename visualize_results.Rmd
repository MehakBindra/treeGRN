...
rdf$MR <- factor(rdf$MR, levels = c(5,40,100), labels = c("MR=5", "MR=40", "MR=100"))
...
# Genie3
MEAN_AUROC.g <- c(0.661429878034267, 0.84640849339549, 0.836797176757893)
```

Plot Global AUPRC for all conditions
```{r fig.width=4}
for (i in seq_along(MR)) {
  ...
  plot(p)
  filename <- paste0("results/", mr , "_AUPRC" , "v2.png")
  ggsave(filename, plot = p)
}
```

Plot Aggregated (mean, std) AUPRC for all conditions
...
for (i in seq_along(MR)) {
  ...
  plot(p)
  filename <- paste0("results/", mr , "_MEAN_AUPRC" , "v2.png")
  ggsave(filename, plot = p)
}
```


Plot Global AUROC for all conditions
...
for (i in seq_along(MR)) {
  ...
  plot(p)
  filename <- paste0("results/", mr , "_AUROC" , "v2.png")
  ggsave(filename, plot = p)
}
```


Plot Aggregated (mean, std) AUROC for all conditions
...
for (i in seq_along(MR)) {
  ...
  p <- rdf %>%
    ...
    geom_errorbar(aes(ymin=MEAN_AUROC-STD_AUROC, ymax=MEAN_AUROC+STD_AUROC), 
                  width=0.4, size = 0.5, position=pd, alpha = 1) +
    geom_point(aes(shape = N_EST), position = pd, size = 3) +
    # geom_point(aes(y = AUROC),position = pd,  size = 2)+
    geom_hline(yintercept = baseline_value, linetype = "dashed", color = "black", alpha = 0.5) + 
        geom_hline(yintercept = MEAN_AUROC.g[i], linetype = "dashed", color = "green", alpha = 0.5) + 
    scale_shape_manual(values=c(3,4,8))+
    theme_pubr() + 
    scale_color_brewer(palette = 'Set1') +
    facet_wrap(MR~., ncol = 1, scales = "free_y") +
  ...
  plot(p)
  filename <- paste0("results/", mr , "_MEAN_AUROC" , "v2.png")
  ggsave(filename, plot = p)
}
```
